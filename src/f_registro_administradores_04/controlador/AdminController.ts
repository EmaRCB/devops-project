...
import { ParseStatus, date } from "zod";
...
async function EnviarCodigoInvitacion(correo: string, codigoInvitacion: string) {
   try {
...
async function PreRegistro(req: any, res: any) {
   ...
   try {
      ...
      const response = jsonResponse(200, 'Codigo de invitacion enviado correctamente', { correo });
...
async function RegistrarAdministrador(req: any, res: any) {
   ...
   try {
      ...
      const response = jsonResponse(201, 'Administrador registrado correctamente', nuevoAdministrador);
...
async function EliminarAdministrador(req: any, res: any) {
   ...
   try {
      ...
      if (!adminExiste) {
         const response = jsonResponse(404, 'El administrador indicado no existe', id);
      ...
      const response = jsonResponse(200, 'El administrador indicado ha sido eliminado', id);
      res.status(200).json(response);
   } catch (error) {
...
async function ActualizarAdministrador(req: any, res: any) {
   ...
   try {
      ...
      if (!adminExiste) {
         const response = jsonResponse(404, 'El administador proporcionado no existe', null);
         return res.status(404).json(response);
      }
      ...
      const response = jsonResponse(200, 'El administrador se ha actualizado correctamente', adminActualizado);
      res.status(200).json(response);
...
async function ObtenerAdministrador(req: any, res: any) {
   ...
   try {
      ...
      if (!administrador) {
         const response = jsonResponse(404, 'El administrador especificado no existe', null);
         return res.status(404).json(response);
      } else {
...
export { PreRegistro, RegistrarAdministrador, EliminarAdministrador, ActualizarAdministrador, ObtenerAdministrador, ObtenerAdministradores };
